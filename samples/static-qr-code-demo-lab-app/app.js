...
async function requestProof () {
  ...
  const inviteURL =
    new Promise(function (resolve, reject) {
      inviteURLMap.set(relationshipDID, resolve)
    })
  const inviteURLValue = await inviteURL
  return (inviteURLValue)
}

async function validateProof (validationResult) {
  const requestProof =
    new Promise(function (resolve, reject) {
      proofRequestMap.set(relationshipDID, resolve)
    })
  proofRequestMap.get(relationshipDID)(validationResult)
...
app.get('/users', function (req, res) {
  ...
  res.render('users', { usersDataMap: userDataMap })
})

// Add test sample id
app.post('/user-registration/:did', function (req, res) {
  const testSampleId = req.body.test_sample
  const did = req.params.did
  const userData = userDataMap.get(did)
  userDataMap.set(did, {
...
// Change test sample id
app.post('/user-update/:did', function (req, res) {
  const testSampleId = req.body.test_sample
  const did = req.params.did
  const userData = userDataMap.get(did)
  const oldTestSampleId = userData.testSampleId
  userDataMap.set(did, {
    givenNames: userData.givenNames,
    lastName: userData.lastName,
    passportNumber: userData.passportNumber,
  ...
  const now = new Date().toISOString()
  didTestSampeMap.delete(oldTestSampleId)
  didTestSampeMap.set(testSampleId, { did: did, taken_date: now })
  currentRelationshipDID = null
  res.redirect('/users')
})
...
app.post('/send-test-results', async (req, res) => {
  ...
  const userData = userDataMap.get(values[0])
  ...
  userDataMap.set(values[0], {
    givenNames: userData.givenNames,
    lastName: userData.lastName,
    passportNumber: userData.passportNumber,
    testSampleId: userData.testSampleId,
    resultIssued: true
  })
...
// Return invite URL when a user clicks on deep link
app.get('/generate-proof-request/', async (req, res) => {
  const inviteurl = await requestProof()
...
app.post('/webhook', async (req, res) => {
  ...
  switch (message['@type']) {
    ...
    case 'did:sov:BzCbsNYhMrjHiqZDTUASHg;spec/present-proof/1.0/presentation-result':
      ...
      const verificationResult = await validateProof(message.verification_result)
    ...
    case 'did:sov:BzCbsNYhMrjHiqZDTUASHg;spec/present-proof/1.0/protocol-invitation':
      inviteURLMap.get(relationshipDID)(message.inviteURL)
      break
